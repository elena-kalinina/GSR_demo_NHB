function get_protocol_data_test(SubjectID, ExpType, sessionN, cfg)

%[onsets, control] = get_protocol_data(SubjectID, ExpType, sessionN)
if nargin<4
    cfg=[];
end
...
%cfg.protocolpath='/home/elenka/Documents/MATLAB/GSR_emotion';
...
if sessionN > 1
    ...
    myonsets_file=sprintf('%s_%s_o_%d.mat', SubjectID, ExpType, sessionN);
    load(myonsets_file, 'real_onsets');
    
    onsets=round(real_onsets(2:length(real_onsets)-1))+cfg.bl_dur+cfg.async;
else
    
 
    onsets=res(:, 2);
    onsets=onsets(code>0); 
...
if cfg.forPSPM
    if sessionN==1
        prepare_for_pspm_3(SubjectID,ExpType,  sessionN, cfg);
    else
        prepare_for_pspm_1_3(SubjectID,ExpType,  sessionN, cfg);
    end
end
