function fig = my_plot_scr_glm(modelfile, glm, plotNr)
        ...
        switch plotNr(i)
            ...
            case 5
              ...
              %  fig(5).ax(1).h=axes('Position', [0.05 0.1 0.9 0.8]);
                ...
                fig(5).ax(1).h=axes('Position', [0.07 0.14 0.9 0.8]);
               
                fig(5).p = plot(glm.resp, 'LineWidth', 8);
                 legend('neg', 'neutr')
              %  legend(glm.reconnames); legend boxoff;
                fig(5).title=get(fig(5).ax(1).h, 'Title');
              %  set(fig(5).ax(1).h, 'TickDir', 'out', 'YTick',[],  'FontWeight', 'Bold', 'FontSize', 20,  'TickLength', [0.005 0.025]);
             %   set(fig(5).ax(1).h, 'TickDir', 'out', 'FontWeight', 'Bold', 'FontSize', 20,  'TickLength', [0.005 0.025]);
                set(fig(5).ax(1).h, 'TickDir', 'out', 'FontSize', 20,  'TickLength', [0.005 0.025]);
                set(fig(5).ax(1).h, 'xtick', 0:100:900)
            oldxlabels=get(fig(5).ax(1).h,'xtick');
            newxlabels=oldxlabels/10;
            set(fig(5).ax(1).h,'xticklabel',newxlabels)
                xlabel('Time (sec)')
                ylabel('Response') 
                grid on
               % set(fig(5).title, 'String', sprintf('Estimated responses per condition: %s', filename), 'FontWeight', 'Bold', 'FontSize', 20, 'Interpreter', 'none');
              %  set(fig(5).title, 'String', 'Estimated responses per condition', 'FontWeight', 'Bold', 'FontSize', 20, 'Interpreter', 'none');
        end
    end
end