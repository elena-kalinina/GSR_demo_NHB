function [modelfile, glm] = my_scr_glm(SubjID, datafile, cfg)
...
cfg.DataPath= '/home/elenka/Documents/MATLAB/MyGSRData/'; % 'samples', 'markers'
end

if ~isfield(cfg, 'modelfile')
cfg.modelfile= sprintf('%s%s_model.mat', cfg.DataPath, SubjID); % 'samples', 'markers'
end
...
cfg.timing= {sprintf('%s%s_pspm_design_%d.mat', cfg.DataPath, SubjID, 1), sprintf('%s%s_pspm_design_%d.mat', cfg.DataPath, SubjID, 2)}; % 'samples', 'markers'
end
...
for iFile = 1:nFile
    [sts, infos, data] = scr_load_data(cfg.datafile{iFile}, cfg.channel);
    if sts < 1, return; end;
    y{iFile} = data{1}.data(:);
