function [sts, outdata, newsr] = myscr_prepdata(data, filt, cfg)
...
%
% FORMAT [sts, data, newsr] = scr_prepdata(data, filt)
% 
...
%
% used by scr_glm, scr_sf, scr_pulse_convert, scr_dcm, scr_pp
%
%__________________________________________________________________________
% PsPM 3.0
% (C) 2008-2015 Dominik R Bach (Wellcome Trust Centre for Neuroimaging)
% $Id: scr_prepdata.m 701 2015-01-22 14:36:13Z tmoser $  
% $Rev: 701 $
...
% modno = 1; %find(strcmpi(cfg.modality, {settings.glm.modality}));
...
if ~ischar(filt.lpfreq) && ~isnan(filt.lpfreq)
    if filt.lpfreq >= nyq 
        ...
        [sts, filt.b, filt.a]=scr_butter(filt.lporder, filt.lpfreq/nyq, 'low');
        if sts == -1, return; end;
        if uni
...
if ~ischar(filt.down) && filt.sr ~= filt.down
    ...
    if freqratio == ceil(freqratio) % NB isinteger might not work for some values
        ...
        [sts, data] = scr_downsample(data, freqratio);
        if sts == -1, errmsg = 'for an unknown reason in scr_downsample.'; end;
        newsr = filt.down;
    elseif cfg.settings.signal ==1
         ...
         if filt.sr == floor(filt.sr) && filt.down == floor(filt.down)
            ...
            newsr = filt.down;
             ...
             newsr = round(filt.sr * altdownsr/altsr)
