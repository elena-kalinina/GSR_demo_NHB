function outfile=my_scr_import(datafile, cfg)

% SCR_IMPORT imports scr data from different formats and writes them to a
%       file to the same path
%
% FORMAT:
% OUTFILE=SCR_IMPORT(datafile, datatype, import, options)
%
% datafile: file containing the scr data, or cell array of files
% datatype: supported datatypes are defined in scr_init (see manual)
%
% import:   a cell array of struct with one job (imported channel) per cell
%           mandatory fields for all data types and each job:
%           - .type: not all data types support all channel types
%           mandatory fields for some data types and each channel:
%           - .sr: sampling rate for waveform, or timeunit in s for event channels
%           - .channel: channel or column number in the original file
%           optional fields for some data types and channel types:
%           - .transfer: name of a .mat file containing values for
%              the transfer function, OR a struct array containing the values
%              OR 'none', when no conversion is required
%              (c and optional Rs and offset, see scr_transfer_function for
%              more information)
%           - .denoise: for marker channels in CED spike format (recorded 
%              as 'level'), filters out markers duration longer than the 
%              value given here (in ms) 
% options:  options.overwrite - overwrite existing files by default
%
% RETURNS
% a .mat file (or cell array of files) on the input file path containing scr
% and event info
%__________________________________________________________________________
% PsPM 3.0
% (C) 2008-2015 Dominik R Bach (Wellcome Trust Centre for Neuroimaging)

% $Id: scr_import.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $

% -------------------------------------------------------------------------
% DEVELOPERS NOTES: Structure of SCRalyze import
%
% scr_import is a general function for handling of import jobs. It checks
% the import job, calls a datatype-specific function to extract data from
% the file, then calls channel-specific functions to convert the data,
% writes them to file, and checks the consistency of the output file using
% scr_load_data
%
% Guideline for new data type functions:
% - functions are named as 'scr_get_datatype' and called
%     [sts, import, sourceinfo] = scr_get_datatype(filename, import)
% - data type must be described in scr_init - see there for details
% - the function needs to take an import job and add, or each job, fields
% -- .data - the actual data
% -- .sr   - the sample rate for this channel (only if enabled in scr_init)
% - optional fields
% -- .marker - for marker channels (see scr_get_marker)
% -- .markerinfo - same
% -- .minfreq - minimum frequency for pulse channels
% -- .units - if data units are defined by the recording software
%
% - sourceinfo contains information on the source file, with field
%        -- .chan - a cell of string descriptions of the imported source
%                    channels, e. g. names, or numbers
%       and any optional fields that will be added to infos.source (e. g.
%           recording date & time, and others)
%
% Guideline for new channel functions:
% - functions are named as 'scr_get_channeltype' and called
%      [sts, data] = scr_get_channeltype(import)
% - see scr_load_data for the required structure of 'data'
%
% Notes for multiple blocks:
% file formats that support multiple block storage within one file can
% return cell arrays import{1:blkno} and sourceinfo{1:blkno}; SCRalyze will
% save individual files for each block, with a filename 'scr_fn_blk0x.mat'
% -------------------------------------------------------------------------

if nargin <2
    cfg=[];
end

if ~isfield(cfg, 'datatype')
    
    cfg.datatype='mat';
       
end

import =[];
import.sr=cfg.rec_sr;
import.type='scr';
import.channel=1;
import.transfer='none';

if ~isfield(cfg, 'import')
    cfg.import={import};
    
end

if ~isfield(cfg, 'options')
    
    cfg.options=[];
    cfg.options.overwrite=0; %1 for overwrite
    
end


% initialise
% -------------------------------------------------------------------------
global settings;
if isempty(settings), scr_init; end;
outfile = [];

% input argument check & transform
% -------------------------------------------------------------------------
% if nargin<1
%     warning('ID:invalid_input', 'No input file'); return;
% elseif ~iscell(datafile) && ~ischar(datafile)
%     warning('ID:invalid_input', 'Input file needs to be a string or cell array'); return;
% elseif nargin<2
%     warning('ID:invalid_input', 'No data type'); return;
% elseif ~ischar(cfg.datatype)
%     warning('ID:invalid_input', 'Data type needs to be a string'); return;
% elseif sum(strcmpi(cfg.datatype, {settings.import.datatypes.short})) == 0
%     warning('ID:invalid_channeltype', 'Data type (%s) not recognised', cfg.datatype); return;
% elseif nargin<3
%     warning('ID:invalid_input', 'No import job'); return;
% elseif ~iscell(cfg.import)
%     if isstruct(cfg.import) && numel(cfg.import) == 1
%         import = {cfg.import};
%     else
%         warning('ID:invalid_input', 'Import needs to be a cell array of struct, or a single struct'); return;
%     end;
% end;

% check options ---
try
    if cfg.options.overwrite ~= 1, cfg.options.overwrite = 0; end;
catch
    cfg.options.overwrite = 0;
end;

% convert data files ---
if iscell(datafile)
    D=datafile;
else
    D={datafile};
end;
clear datafile


% import job check
% -------------------------------------------------------------------------

% determine datatype ---
cfg.datatype = find(strcmpi(cfg.datatype, {settings.import.datatypes.short}));

% check number of jobs ---
% more than one job when only on is allowed?
if (~settings.import.datatypes(cfg.datatype).multioption) && (numel(cfg.import) > 1)
    % two jobs when one is an automatically assigned marker channel?
    if ~(settings.import.datatypes(cfg.datatype).automarker && numel(cfg.import) == 2 && ...
            any(strcmpi({cfg.import{1}.type, cfg.import{2}.type}, 'marker')))
        warning('ID:ivalid_import_struct', 'Only one data channel can be imported at a time for data type ''%s''.\n', ...
            settings.import.datatypes(cfg.datatype).long); return;
    end;
end;

% check each job ---
for k = 1:numel(cfg.import)
    % channel type specified?
    if ~isfield(cfg.import{k}, 'type')
        warning('ID:ivalid_import_struct', 'No type given for import job %2.0f.\n', k); return;
        % channel type allowed for this datatype?
    elseif sum(strcmpi(cfg.import{k}.type, settings.import.datatypes(cfg.datatype).chantypes)) == 0
        warning('ID:ivalid_import_struct', 'Channel type ''%s'' in import job %2.0f is not supported for data type %s.\n', ...
            cfg.import{k}.type, k, settings.import.datatypes(cfg.datatype).long); return;
        % sample rate given or automatically assigned?
    elseif ~isfield(cfg.import{k}, 'sr') && ~settings.import.datatypes(cfg.datatype).autosr
        warning('ID:ivalid_import_struct', 'Sample rate needed for import job %02.0f of type %s.\n', ...
            k, cfg.import{k}.type); return;
        % sample rate given AND automatically assigned? If yes, remove and say so.
    elseif isfield(cfg.import{k}, 'sr') && settings.import.datatypes(cfg.datatype).autosr
        cfg.import{k} = rmfield(cfg.import{k}, 'sr');
        fprintf('Sample rate for import job %02.0f of type %s discarded - will be automatically assigned.\n', ...
            k, cfg.import{k}.type);
    end;
    % marker channel in data format where no channel name is needed?
    if strcmpi(cfg.import{k}.type, 'marker') && settings.import.datatypes(cfg.datatype).automarker
        cfg.import{k}.channel = 1;
    end;
    % channel number given? If not, set to zero, or assign automatically and display.
    if ~isfield(cfg.import{k}, 'channel')
        if settings.import.datatypes(cfg.datatype).searchoption
            cfg.import{k}.channel = 0;
        else
            cfg.import{k}.channel = k;
            fprintf('Assigned channel/column %1.0f to import job %1.0f of type %s.\n', k, k, cfg.import{k}.type);
        end;
    end;
    % assign channel type number
    cfg.import{k}.typeno = find(strcmpi(cfg.import{k}.type, {settings.chantypes.type}));
end;

% loop through data files
% -------------------------------------------------------------------------
for d = 1:numel(D)
    fprintf('Importing %s ...', D{d});
    % pass over to import function if datafile exists, otherwise next file
    if exist(D{d}, 'file')
        [sts, import, sourceinfo] = feval(settings.import.datatypes(cfg.datatype).funct, D{d}, cfg.import);
    else
        sts = -1; warning('ID:nonexistent_file', '\nDatafile (%s) doesn''t exist', D{d});
    end;
    if sts == -1, fprintf('\nImport unsuccesful for file %s.\n', D{d}); break; end;
    
    % split blocks if necessary ---
    if iscell(sourceinfo)
        blkno = numel(sourceinfo);
    else
        blkno = 1;
        import = {import};
        sourceinfo = {sourceinfo};
    end;
    
    for blk = 1:blkno
        % convert data into desired channel type format ---
        data = cell(numel(import{blk}), 1);
        for k = 1:numel(import{blk})
            if ~isfield(import{blk}{k}, 'units'), import{blk}{k}.units = 'unknown'; end;
            chantype = find(strcmpi(import{blk}{k}.type, {settings.chantypes.type}));
            [sts(k), data{k}] = feval(settings.chantypes(chantype).import, import{blk}{k});
            if isfield(import{blk}{k}, 'minfreq'), data{k}.header.minfreq = import{blk}{k}.minfreq; end;
        end;
        
        if any(sts == -1), fprintf('\nData conversion unsuccesful for job %02.0f file %s.\n', ...
                find(sts == -1), D{d}); break; end;
        
        % collect infos and save ---
        [pth, fn, ext]     = fileparts(D{d});
        infos.source = sourceinfo{blk};
        infos.source.type = settings.import.datatypes(cfg.datatype).long;
        infos.source.file = D{d};
        infos.importdate = date;

        % align data length ---
        [sts, data, duration] = scr_align_channels(data);
        if sts == -1, fprintf('\nData alignment unsuccesful for file %s.\n', D{d}); break; end;
        infos.duration   = duration;
        infos.durationinfo = 'Recording duration in seconds';
        data = data(:);

        % save file ---
        if blkno == 1
            outfile{d, blk}=fullfile(pth, [settings.import.fileprefix, fn, '.mat']);
        else
            outfile{d, blk}=fullfile(pth, sprintf('%s%s_blk%02.0f.mat', settings.import.fileprefix, fn, blk));
        end;
        infos.importfile = outfile{d};
        clear savedata
        savedata.data = data; savedata.infos = infos; savedata.options = cfg.options;
        sts = scr_load_data(outfile{d, blk}, savedata);
        if sts ~= 1
            warning('Import unsuccessful for file %s.\n', D{d}); 
            outfile{d, blk} = [];
        end;
    end;
    fprintf(' done.\n');
    % convert import cell back and remove data ---
    import = {cfg.import{1}};
    for k = 1:numel(import)
        if isfield(import{k}, 'data'), import{k} = rmfield(import{k}, 'data'); end;
    end;
end;
